...
async def parse_and_store(session: AsyncSession) -> int:
    ...
    try:
        # Контекстный менеджер гарантирует закрытие соединений после завершения
        async with httpx.AsyncClient(timeout=timeout) as client:
            page = 1
            while True:
                payload = await fetch_page(client, page)

                # Преобразуем элементы API во внутренний формат
                parsed_payloads = []
                for item in payload.items:
                    city_name = item.city.name.strip() if item.city else None  # защита от None
